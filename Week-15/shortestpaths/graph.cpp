...
namespace data {
    ...
    void graph::add_edge(const std::string &from, const std::string &to, long int weight) {
        // lookup source
        ...
        // add edge (source -> to)
        m_vertices[source_id].m_edges.emplace_back(graph::edge{*this, source_id, target_id, weight});

        if (!m_directed) {
            // add an edge from 'to' to from
            m_vertices[target_id].m_edges.emplace_back(graph::edge{*this, target_id, source_id, weight});
        }
        m_num_edges++;
    }
    ...
    const graph::edge& graph::find_edge(const std::string &from, const std::string &to) const {
        auto &source = m_vertices[find_id(from)];
        for (auto & edge : source.m_edges) {
            if (edge.target().name() == to) return edge;
        }
    ...
    graph graph::load(const std::string &filename) {
        ...
        if (!input) throw std::invalid_argument{"Could not open file " + filename};
        std::string line;
        input >> line;
        bool directed;
    ...
    graph &graph::operator=(const graph &other) {
        ...
        for (auto &v: other.m_vertices) {
            for (auto &vw: v.m_edges) {
                add_edge(v.name(), vw.target().name(), 0);
            }
    ...
    bool graph::has_edge(const std::string &from, const std::string &to) const {
        ...
        auto &source = m_vertices[find_id(from)];
        for (auto & edge : source.m_edges) if (edge.target().name() == to) return true;
        return false;
    }

    bool graph::has_vertex(const std::string &name) const {
        return m_name_to_id.contains(name);
    }
    ...
    void graph::edge::set_colour(data::colour colour) {
        m_colour = colour;
        if (!m_graph->m_directed) {
            m_graph->find_edge(target().name(), m_graph->m_vertices[m_source].name()).m_colour = colour;
        }
    }

    graph::edge::edge(graph &g, size_t source, size_t adjacent_vertex, long int weight)
            : m_graph{&g},
              m_source{source},
    ...
    long int graph::edge::weight() const {
        return m_weight;
    }
